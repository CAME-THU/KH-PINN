...
class Case:
    def __init__(self, args):
        ...
        self.names = {
            "independents": ["x", "y", "t"],
            "dependents": ["u", "v", "p", "c"],
            "equations": ["incompress", "momentum_x", "momentum_y", "scalar"],
            "ICBCOCs": []}
        ...
        # ----------------------------------------------------------------------
        # load data
        # data_path = f"../data/data_drho0.0_Re{args.Re_str}_512x1024x101.h5"
        data_path = f"../data/data_drho0.0_Re{args.Re_str}_256x512x101.h5"
        print("loading data...")
    ...
    # ----------------------------------------------------------------------
    # define pde
    def pde(self, xyt, uvpc):
        args = self.args
        # x, y, t = xyt[:, 0:1], xyt[:, 1:2], xyt[:, 2:]
        u, v, p, c = uvpc[:, 0:1], uvpc[:, 1:2], uvpc[:, 2:3], uvpc[:, 3:]
        scale_u, scale_v, scale_p, scale_c = args.scales["u"], args.scales["v"], args.scales["p"], args.scales["c"]
        ...
        u_x = dde.grad.jacobian(uvpc, xyt, i=0, j=0)
        u_y = dde.grad.jacobian(uvpc, xyt, i=0, j=1)
        u_t = dde.grad.jacobian(uvpc, xyt, i=0, j=2)
        u_xx = dde.grad.hessian(uvpc, xyt, component=0, i=0, j=0)
        u_yy = dde.grad.hessian(uvpc, xyt, component=0, i=1, j=1)

        v_x = dde.grad.jacobian(uvpc, xyt, i=1, j=0)
        v_y = dde.grad.jacobian(uvpc, xyt, i=1, j=1)
        v_t = dde.grad.jacobian(uvpc, xyt, i=1, j=2)
        v_xx = dde.grad.hessian(uvpc, xyt, component=1, i=0, j=0)
        v_yy = dde.grad.hessian(uvpc, xyt, component=1, i=1, j=1)

        p_x = dde.grad.jacobian(uvpc, xyt, i=2, j=0)
        p_y = dde.grad.jacobian(uvpc, xyt, i=2, j=1)

        c_x = dde.grad.jacobian(uvpc, xyt, i=3, j=0)
        c_y = dde.grad.jacobian(uvpc, xyt, i=3, j=1)
        c_t = dde.grad.jacobian(uvpc, xyt, i=3, j=2)
        c_xx = dde.grad.hessian(uvpc, xyt, component=3, i=0, j=0)
        c_yy = dde.grad.hessian(uvpc, xyt, component=3, i=1, j=1)
        ...
        scalar = c_t + u * c_x + v * c_y - D * (c_xx + c_yy)
        ...
        return [incompress, momentum_x, momentum_y, scalar]

    def omega(self, xyt, uvpc):
        u_y = dde.grad.jacobian(uvpc, xyt, i=0, j=1)
        v_x = dde.grad.jacobian(uvpc, xyt, i=1, j=0)
        return v_x - u_y
    ...
    def define_icbcocs(self):
        ...
        # TODO: update scales
        bc_u_lr = ScaledPeriodicBC(geom_time, 0, lambda _, on_bdr: on_bdr, component=0, scale=scale_u)
        bc_v_lr = ScaledPeriodicBC(geom_time, 0, lambda _, on_bdr: on_bdr, component=1, scale=scale_v)
        bc_p_lr = ScaledPeriodicBC(geom_time, 0, lambda _, on_bdr: on_bdr, component=2, scale=scale_p)
        bc_c_lr = ScaledPeriodicBC(geom_time, 0, lambda _, on_bdr: on_bdr, component=3, scale=scale_c)

        bc_u_du = ScaledPeriodicBC(geom_time, 1, lambda _, on_bdr: on_bdr, component=0, scale=scale_u)
        bc_v_du = ScaledPeriodicBC(geom_time, 1, lambda _, on_bdr: on_bdr, component=1, scale=scale_v)
        bc_p_du = ScaledPeriodicBC(geom_time, 1, lambda _, on_bdr: on_bdr, component=2, scale=scale_p)
        bc_c_du = ScaledPeriodicBC(geom_time, 1, lambda _, on_bdr: on_bdr, component=3, scale=scale_c)

        # ic_u = dde.icbc.IC(geom_time, self.func_u, lambda _, on_ini: on_ini, component=0)
        # ic_v = dde.icbc.IC(geom_time, self.func_v, lambda _, on_ini: on_ini, component=1)
        # ic_p = dde.icbc.IC(geom_time, self.func_p, lambda _, on_ini: on_ini, component=2)
        # ic_c = dde.icbc.IC(geom_time, self.func_c, lambda _, on_ini: on_ini, component=3)

        if args.bc_type == "soft":
            self.icbcocs += [bc_u_lr, bc_v_lr, bc_p_lr, bc_c_lr, bc_u_du, bc_v_du, bc_p_du, bc_c_du, ]
            self.names["ICBCOCs"] += ["BC_u_lr", "BC_v_lr", "BC_p_lr", "BC_c_lr", "BC_u_du", "BC_v_du", "BC_p_du",
                                      "BC_c_du", ]
        else:  # "none"
            pass

        # if ic_type == "soft":
        #     self.icbcocs += [ic_u, ic_v, ic_p, ic_c]
        #     self.names["ICBCOCs"] += ["IC_u", "IC_v", "IC_p", "IC_c"]
        # else:  # "none", "hard"
        #     pass
        ...
        if args.oc_type == "soft":
            ...
            # nx_ob, ny_ob, nt_ob = 32, 64, 51
            # nx_ob, ny_ob, nt_ob = 64, 128, 101
            # nx_ob, ny_ob, nt_ob = args.nx_ob, args.ny_ob, args.nt_ob
            ...
            # bachsize_oc = 2000
            # bachsize_oc = n_ob // 4
            bachsize_oc = min(16 * 32 * 51 // 4, n_ob)
            # oc_u = ScaledPointSetBC(ob_xyt, ob_u, component=0, batch_size=bachsize_oc, scale=args.scales["u"])
            # oc_v = ScaledPointSetBC(ob_xyt, ob_v, component=1, batch_size=bachsize_oc, scale=args.scales["v"])
            ...
            oc_p = ScaledPointSetBC(ob_xyt, ob_p, component=2, batch_size=bachsize_oc, scale=args.scales["p"])
            oc_c = ScaledPointSetBC(ob_xyt, ob_c, component=3, batch_size=bachsize_oc, scale=args.scales["c"])
            self.icbcocs += [oc_u, oc_v, oc_p, oc_c, ]
            self.names["ICBCOCs"] += ["OC_u", "OC_v", "OC_p", "OC_c", ]
        else:  # "none"
            ob_xyt = np.empty([1, 3])
            n_ob = 0
